...
#include "edb_key_demo_t.h"

#include "misc.h"
#include "edb_key_demo_model_data.h"
#include "common.h"
...
int ecall_nn(uint8_t *img, size_t size, size_t w, size_t h, size_t n)
{
    ...
    tf.model = ::tflite::GetModel(g_edb_key_demo_model_data);
...
oe_result_t infer(struct tflite_model *tf, uint8_t *img, size_t w, size_t h, size_t n)
{
	...
	if (out[0] > 0.95)
		ocall_class_result(&ret, LOGO_B, out[0]); // R
	else if (out[1] > 0.95)
		ocall_class_result(&ret, LOGO_U, out[1]); // B
	else if (out[2] > 0.95)
		ocall_class_result(&ret, LOGO_R, out[2]); // Y
	else if (out[3] > 0.95)
		ocall_class_result(&ret, LOGO_Y, out[3]); // U
	else
